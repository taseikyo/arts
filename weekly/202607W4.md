...
本文总字数 6931 个，阅读时长约：25 分 12 秒，统计数据来自：[汤圆工具箱](https://zishutongji.zaixianapp.cn/)。
...
本周最爱 🎵 ：[红色高跟鞋 - 蔡健雅](https://music.163.com/#/song?id=208902)

<iframe frameborder="no" border="0" marginwidth="0" marginheight="0" width=330 height=86 src="//music.163.com/outchain/player?type=2&id=208902&auto=1&height=66"></iframe>

## Table of Contents

- [algorithm 🔝](#algorithm-)
    - [1. 49.字母异位词分组](#1-49字母异位词分组)
        - [49-Desc](#49-desc)
        - [49-Code](#49-code)
    - [2. 128.最长连续序列](#2-128最长连续序列)
        - [128-Desc](#128-desc)
        - [128-Code](#128-code)
- [review 🔝](#review-)
    - [1. 并查集（Union-Find / Disjoint Set Union，DSU）详解](#1-并查集union-find--disjoint-set-uniondsu详解)
        - [什么是并查集](#什么是并查集)
        - [为什么需要并查集](#为什么需要并查集)
        - [核心思想](#核心思想)
        - [基本结构](#基本结构)
        ...
        - [优化一：路径压缩](#优化一路径压缩)
        - [优化二：按秩合并](#优化二按秩合并)
        - [完整 Python 实现](#完整-python-实现)
        - [时间复杂度](#时间复杂度)
        - [并查集和 DFS/BFS 的区别](#并查集和-dfsbfs-的区别)
        - [总结](#总结)
- [tip 🔝](#tip-)
    - [1. Git HTTP 代理与 SSH 代理原理说明](#1-git-http-代理与-ssh-代理原理说明)
        - [1. 问题背景](#1-问题背景)
        - [2. Git 的两种连接方式](#2-git-的两种连接方式)
        - [3. 两种代理配置的区别](#3-两种代理配置的区别)
        - [4. ProxyCommand 原理](#4-proxycommand-原理)
        - [5. 两种代理所在层级](#5-两种代理所在层级)
        - [6. 为什么 SSH 不读取 gitconfig？](#6-为什么-ssh-不读取-gitconfig)
        - [7. 推荐配置方式](#7-推荐配置方式)
        - [8. GitHub SSH 443 端口方案](#8-github-ssh-443-端口方案)
        - [9. SSH 连接复用优化](#9-ssh-连接复用优化)
        - [10. 总结](#10-总结)
    - [2. 什么是左侧交易和右侧交易？（雪球 | 中文）](#2-什么是左侧交易和右侧交易雪球--中文)
    - [3. 布隆过滤器（Bloom Filter）原理、工业应用与优化技术详解](#3-布隆过滤器bloom-filter原理工业应用与优化技术详解)
        - [1. 什么是布隆过滤器](#1-什么是布隆过滤器)
        - [2. 基本组成](#2-基本组成)
        - [3. 布隆过滤器工作原理](#3-布隆过滤器工作原理)
        - [4. 查询过程](#4-查询过程)
        - [5. 为什么会误判？](#5-为什么会误判)
        - [6. 数学原理](#6-数学原理)
        - [7. 工业为什么使用 Bloom Filter？](#7-工业为什么使用-bloom-filter)
        - [8. 业界应用场景](#8-业界应用场景)
        - [9. Bloom Filter优化版本](#9-bloom-filter优化版本)
        - [10. Bloom Filter在大模型时代的位置](#10-bloom-filter在大模型时代的位置)
        - [11. Python实现一个基础版本的 Bloom Filter](#11-python实现一个基础版本的-bloom-filter)
        - [12. 总结](#12-总结)
    - [4. 真假阳性阴性详解（True/False Positive/Negative）](#4-真假阳性阴性详解truefalse-positivenegative)
        - [一句话理解](#一句话理解)
        - [四种情况总表](#四种情况总表)
        - [1. 真阳性 TP（True Positive）](#1-真阳性-tptrue-positive)
        - [2. 假阳性 FP（False Positive）](#2-假阳性-fpfalse-positive)
        - [3. 真阴性 TN（True Negative）](#3-真阴性-tntrue-negative)
        - [4. 假阴性 FN（False Negative）](#4-假阴性-fnfalse-negative)
        - [机器学习中的理解](#机器学习中的理解)
        - [常见指标](#常见指标)
        - [总结](#总结)
- [share 🔝](#share-)
    - [1. 以终为始 | Begin with the end in mind](#1-以终为始--begin-with-the-end-in-mind)

## algorithm [🔝](#weekly-110)

### 1. [49.字母异位词分组](https://leetcode.cn/problems/group-anagrams/description)
...
[49.group-anagrams](../code/leetcode2026/49.group-anagrams.go)
...
### 2. [128.最长连续序列](https://leetcode.cn/problems/longest-consecutive-sequence/description)
...
### 2. [什么是左侧交易和右侧交易？（雪球 | 中文）](https://xueqiu.com/9130951873/243826713)
